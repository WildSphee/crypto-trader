...
from features.on_chain_data import get_btc_onchain_smoothed, SUPPORTED_INTERVALS, BTC_METRICS
...
def binance_klines_payload():
    ...
    def make(symbol, interval, start_ts, end_ts):
        ...
        for t in idx:
            ...
            rows.append([
                open_ms, str(price), str(price*1.01), str(price*0.99), str(price*1.005),
                "10.0", open_ms+int(60e3), "0", 100, "0", "0", "0"
            ])
            price *= 1.001
        return rows
    return make
...
def test_onchain_grid_and_columns(monkeypatch, charts_payload_three_days, binance_klines_payload, interval):
...
def test_onchain_include_price_joins(monkeypatch, charts_payload_three_days, binance_klines_payload, interval):
    _mock_requests_get(monkeypatch, charts_payload_three_days, binance_klines_payload)
...
def test_onchain_bad_interval_raises(monkeypatch, charts_payload_three_days, binance_klines_payload, bad_interval):
    _mock_requests_get(monkeypatch, charts_payload_three_days, binance_klines_payload)
